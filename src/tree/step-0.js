...
const Insert = (node, key, value, root = node) => {
  ...
  if (node.type === 'Leaf') {
    ...
    const last = i === node.keys.length

    node.keys.splice(i, 0, key)
    node.values.splice(i, 0, value)

    if (last) {
      Replace(root, node.keys[node.keys.length - 2], key)
    }
    ...
    return Insert(node.links[i === node.keys.length ? node.keys.length - 1 : i], key, value, root)
  }
}

// replace Node key with new max
const Replace = (root, key, max) => {
  if (root.type === 'Leaf') {
    return
  }

  const i = binarySearch(root.keys, key)

  if (root.keys[i] === key) {
    root.keys[i] = max
  }

  Replace(root.links[i], key, max)
}

const Shift = (node, key, link, root) => {
  // root must be Node and key was lower than root max key
...
const Split = (node, root) => {
  ...
  if (node.type === 'Leaf') {
    ...
    if (node.parent) {
      ...
      node_.keys = [left.keys.slice(-1)[0], right.keys.slice(-1)[0]]
  ...
  if (node.type === 'Node') {
    ...
    const left = NodeT({
      parent: node_,
      keys: node.keys.slice(0, l),
      links: node.links.slice(0, l),
    })

    // update parent
    left.links.forEach((n) => { n.parent = left })
    left.links.forEach((n, i) => i + 1 < left.links.length && (n.next = left.links[i + 1]))
...
// step 0
// keep all keys, len(keys) = len(links)
// `*, k1, *, k2, *, k3`
// if max was updated, all k3 should be updated, see Replace.
