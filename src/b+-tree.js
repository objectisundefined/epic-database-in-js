...
const leaf_node_insert = async (buffer, cell_num, key, value, table) => {
  ...
  if (cell_num < num_cells) {
    ...
    for (let i = num_cells; i > cell_num; i--) {
      leaf_node_cell(buffer, i).write(leaf_node_cell(buffer, i - 1).read())
    }
  }
...
const leaf_node_split_and_insert = async (buffer, cell_num, key, value, table) => {
  ...
  const parent_page_num = node_parent(old_node).read()
  const parent_buffer = await table.pager.page(parent_page_num)
  const new_max = get_node_max_key(old_node) // old_node was splitted, so new_max is the max of the new node

  update_internal_node_key(parent_buffer, old_max, new_max)
  await internal_node_insert(parent_buffer, new_page_num, table)
}
...
const create_new_root = async (table, right_child_page_num) => {
  ...
  node_parent(left_child).write(table.root_page_num)
  node_parent(right_child).write(table.root_page_num)
}
...
const internal_node_child = (buffer, child_num) => {
  ...
  const internal_node_cell_ = (buffer, cell_num) => {
    assert(buffer.length === PAGE_SIZE, `buffer.length: ${buffer.length}`)
  ...
  return internal_node_cell_(buffer, child_num)
}
...
const internal_node_find_child = (buffer, key) => {
  const num_keys = internal_node_num_keys(buffer).read()
...
const internal_node_find = async (buffer, key, pager) => {
  const i = internal_node_find_child(buffer, key)
...
const node_parent = (buffer) => {
  assert(buffer.length === PAGE_SIZE, `buffer.length: ${buffer.length}`)

  const offset = PARENT_POINTER_OFFSET
...
const update_internal_node_key = (buffer, old_key, new_key) => {
  const index = internal_node_find_child(buffer, old_key)
  internal_node_key(buffer, index).write(new_key)
}
...
const internal_node_insert = async (parent_buffer, child_page_num, table) => {
  const child = await table.pager.page(child_page_num)
  const child_max_key = get_node_max_key(child)
  const index = internal_node_find_child(parent_buffer, child_max_key)

  // next max key index also
  const original_num_keys = internal_node_num_keys(parent_buffer).read()
  internal_node_num_keys(parent_buffer).write(original_num_keys + 1)

  if (original_num_keys >= INTERNAL_NODE_MAX_CELLS) {
    throw Error('Need to implement splitting internal node')
  }

  const right_child_page_num = internal_node_right_child(parent_buffer).read()
  const right_child = await table.pager.page(right_child_page_num)
  const right_child_max_key = get_node_max_key(right_child)

  if (child_max_key > right_child_max_key) {
    /* Replace right child */
    internal_node_child(parent_buffer, original_num_keys).write(right_child_page_num)
    internal_node_key(parent_buffer, original_num_keys).write(right_child_max_key)
    internal_node_right_child(parent_buffer).write(child_page_num)
  } else {
    // Make room for the new cell
    for (let i = original_num_keys; i > index; i--) {
      internal_node_child(parent_buffer, i).write(internal_node_child(parent_buffer, i - 1).read())
      internal_node_key(parent_buffer, i).write(internal_node_key(parent_buffer, i - 1).read())
    }
    internal_node_child(parent_buffer, index).write(child_page_num)
    internal_node_key(parent_buffer, index).write(child_max_key)
  }
}
