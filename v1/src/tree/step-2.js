...
const Insert = (node, key, value, root = node) => {
  ...
  if (node.type === 'Leaf') {
    ...
    if (node.keys.length <= Order.Leaf) {
      return root
    }
...
const Shift = (node, key, link, root) => {
  ...
  if (node.keys.length <= Order.Node) {
    return root
  }
...
const Split = (node, root) => {
  ...
  if (node.type === 'Leaf') {
    ...
    if (node.parent) {
      ...
      node.parent.links.forEach((n) => {
        n.next === node && (n.next = left)
      })

      return Shift(node.parent, left.keys[l - 1], left, root)
    } else {
      node.parent = node_
...
// step 2
// modified in place
// leaf's Order and node's Order was separated.
